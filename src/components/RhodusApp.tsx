...
export default function RhodusApp() {
  ...
  const [activeTab, setActiveTab] = useState<'levels' | 'players' | 'custom'>('levels');
    ...
    <div className="flex h-full w-full bg-[#000000] text-white font-mono select-none overflow-hidden">
      ...
      >
        ...
        <div id="panel" className={`panel ${isCountingDown ? 'hidden' : ''}`}>
          ...
          {/* Tabs: Levels / Spieler / Einstellungen */}
          <div className="flex gap-1 p-1 bg-[#181818] border border-[#2a2a2a] rounded-xl">
            {(['levels','players','custom'] as const).map(t => (
              <button key={t} type="button" onClick={() => setActiveTab(t)}
                className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-colors ${activeTab===t?'bg-white text-black':'text-white/50 hover:text-white hover:bg-white/10'}`}>
                {t==='levels'?'Levels':t==='players'?'Spieler':'Setup'}
              </button>
            ))}
          </div>
          ...
          {activeTab==='levels' ? (
            ...
            <div className="level-cards">
              {PLAYERS.map(p => (
                <button key={p.name} type="button" className="level-card !flex-row !items-center gap-3 !py-3" onClick={() => {
                  const ms = parseInt(p.interval) || 1000;
                  setIntervalMs(ms); setTargetFlashes(p.colorChange); setShuffleEnabled(false); setCountdownSeconds(5);
                  beginCountdown();
                }}>
                  <div className="w-10 h-10 rounded-full overflow-hidden shrink-0 bg-white/10 border border-white/10 flex items-center justify-center">
                    {p.avatar ? <img src={p.avatar} alt={p.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" /> : <span className="font-black">{p.name[0]}</span>}
                  </div>
                  <div className="flex flex-col items-start flex-1 min-w-0">
                    <span className="level-title !mb-0 truncate">{p.name}</span>
                    <span className="level-desc truncate">{p.interval} · {p.colorChange} Wechsel · {p.handPreference}</span>
                  </div>
                </button>
              ))}
              <p className="text-[11px] text-white/30 text-center px-2">Tippen startet direkt – Werte aus <code>paul-react/src/lib/players.ts</code></p>
            </div>
          ) : (
            <>
              ...
              <div className="actions">
                ...
                <button id="backToLevelsButton" className="secondary" type="button" onClick={() => setActiveTab('levels')}>
