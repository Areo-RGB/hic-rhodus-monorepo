...
class NearbyManager(
        ...
        override fun onConnectionResult(endpointId: String, resolution: ConnectionResolution) {
            when (resolution.status.statusCode) {
                ...
                ConnectionsStatusCodes.STATUS_CONNECTION_REJECTED -> {
                    ...
                    Log.e(
                        TAG,
                        "Connection error with: $endpointId status: ${resolution.status.statusCode}"
                    )
                    notifyStatus(
                        "error",
                        "Verbindungsstatus: ${resolution.status.statusMessage ?: resolution.status.statusCode}"
                    )
                }
            }
        }
    ...
    @JavascriptInterface
    fun startUnifiedMode(name: String) {
        currentRole = "unified"
        stopAllInternal()
        val client = connectionsClient ?: run {
            notifyStatus(
                "error",
                "Nearby Service nicht verfügbar"
            ); return
        }
        val endpointName = if (name.isNotBlank()) name else "HicRhodusUnified"
        try {
            client.startAdvertising(
                endpointName,
                SERVICE_ID,
                ...
                .addOnSuccessListener {
                    Log.d(
                        TAG,
                        "Unified advertising started"
                    ); notifyStatus("advertising", endpointName)
                }
                .addOnFailureListener { e ->
                    Log.e(
                        TAG,
                        "Unified advertising failed",
                        e
                    ); notifyStatus("error", e.message ?: "advertising failed")
                }
            client.startDiscovery(
                SERVICE_ID,
                endpointDiscoveryCallback,
                ...
                .addOnSuccessListener {
                    Log.d(
                        TAG,
                        "Unified discovery started"
                    ); notifyStatus("discovering", "Unified: suche + sichtbar")
                }
                .addOnFailureListener { e -> Log.e(TAG, "Unified discovery failed", e) }
        } catch (e: SecurityException) {
            notifyStatus("error", "Berechtigungen fehlen"); requestPermissions()
        }
    }
